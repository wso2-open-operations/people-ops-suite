...
import { DataGrid, GridSelectionModel, GridToolbar, GridToolbarExportContainer } from "@mui/x-data-grid";
import { useAppDispatch, useAppSelector } from "@slices/store";
import { showSnackBarMessage } from "@slices/commonSlice/common";
import { fetchOpenParCycle, openParCycle, selectCurrentCycle, selectParCycleState } from "@slices/parCycleSlice";
import { LoadingEffect } from "@components/ui/Loading";
import ErrorComponent from "@components/ui/ErrorComponent";
import QuotaChip from "@components/common/QuotaStatusChip";
import EditQuotaDialog from "@components/common/EditQuotaDialog";
import NoDataView from "@components/common/NoDataView";
import InputDialog from "@components/common/GroupNameInputDialog";
import { RequestState, GroupedTeams, SpecialQuotaTeam, SpecialRatingQuota, PostSpecialQuotaTeam } from "@utils/types";
import { snackMessages, tooltipVisibilityDelay, uiMessages } from "@config/constant";
...
import {
  ...
  resetQuotaSate,
} from "@slices/specialQuotaSlice";
import Chip from "@mui/material/Chip";
import { object, number, string, array } from "yup";
import { useConfirmationModalContext } from "../../../context/DialogContext";
import { fetchConfigurations } from "@slices/metaSlice";
...
export const AssignQuota = () => {
  ...
  const handleGroupCreation = (name: string) => {
    if (selectionModel.length > 0) {
      ...
      dispatch(showSnackBarMessage(snackMessages.success.groupCreated, "success"));
    }
  };
  ...
  const removeGroupFromGroupMap = () => {
    ...
    if (groupToRemove) {
      ...
      dispatch(showSnackBarMessage(snackMessages.success.groupRemoved, "success"));
    }
  };
  ...
  const handleRemoveTeam = () => {
    ...
    dispatch(showSnackBarMessage(snackMessages.success.teamRemoved, "success"));
  ...
  const validateGroupMap = () => {
    if (!isTeamsTableEmpty) {
      dispatch(showSnackBarMessage(snackMessages.error.groupAssignIncomplete, "error"));
      return false;
    }
    try {
      ...
      dispatch(showSnackBarMessage(snackMessages.error.groupValidationFailed, "error"));
      return false;
    }
  };
  ...
  const confirmAndProceed = async () => {
    ...
    if (cycleID === null || cycleID === undefined) {
      dispatch(showSnackBarMessage(snackMessages.error.fetchCurrentCycleDetails, "error"));
      return;
    }
    try {
      ...
      setIsDataSubmitting(false);
      dispatch(showSnackBarMessage(snackMessages.error.common, "error"));
    }
  };
  ...
  const postQuotaData = async (cycleID: number): Promise<boolean> => {
    ...
    if (!validationResult.isValid) {
      dispatch(showSnackBarMessage(snackMessages.error.quotValidationError, "error"));
        ...
        <>
          ...
          {quotaGroupStatus === RequestState.IDLE && !isDataSubmitting && (
            <Box flexDirection={"row"} height={"100%"} zIndex={1000} overflow={"auto"}>
              ...
              >
                <Grid item xs={6} sm={5} alignContent={"center"}>
