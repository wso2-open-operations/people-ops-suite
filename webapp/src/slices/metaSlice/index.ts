...
export const fetchConfigurations = createAsyncThunk("globalConfig/fetchConfigurations", async (_, { dispatch }) => {
  try {
    ...
    const errorMessage = getErrorMessage(error, snackMessages.error.fetchGlobalParConfigs);
    dispatch(
    ...
    try {
      ...
      const message =
        resp.status === HttpStatusCode.Ok
          ? snackMessages.success.updateGlobalParConfigs
          : snackMessages.error.updateGlobalParConfigs;
      ...
      const errorMessage = getErrorMessage(error, snackMessages.error.updateGlobalParConfigs);
      dispatch(
        enqueueSnackbarMessage({
          message: errorMessage,
          type: "error",
...
export const fetchEmployees = createAsyncThunk("globalConfig/fetchEmployees", async (_, { dispatch }) => {
  try {
    ...
    const errorMessage = getErrorMessage(error, snackMessages.error.fetchEmployeesData);
    dispatch(
      enqueueSnackbarMessage({
        message: errorMessage,
        type: "error",
