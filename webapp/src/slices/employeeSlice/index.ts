...
import { fetchParCycleById } from "@slices/parCycleSlice";
  ...
  async (employeeId: string, { dispatch }) => {
    try {
      ...
      const errorMessage = getErrorMessage(
        error,
        snackMessages.error.fetchEmployeeParCycles
      );
  ...
  async (
    ...
    try {
      ...
      const errorMessage = getErrorMessage(
        error,
        values?.parF2fStatus
          ? snackMessages.error.updateF2fStatus
          : snackMessages.error.updateEmployeeParRatings
      );
...
export const updateParTeamIdOfEmployee = createAsyncThunk(
  ...
  async ({ employeeId, parCycleId }: UpdateParTeamIdParam, { dispatch }) => {
    try {
      ...
      const errorMessage = getErrorMessage(
        error,
        snackMessages.error.employeeSyncError
      );
      dispatch(
        enqueueSnackbarMessage({
          message: errorMessage,
      ...
      .addCase(fetchParRatingOfEmployee.rejected, (state, action) => {
        ...
        state.errorMessage =
          typeof action.payload === "string"
            ? action.payload
            : snackMessages.error.fetchEmployeeParRatings;
      })
