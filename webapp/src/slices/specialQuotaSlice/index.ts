...
import { snackMessages } from "@config/constant";
import { sliceErrorMessages } from "@config/constant";
import { enqueueSnackbarMessage } from "@slices/commonSlice/common";
import { createAsyncThunk, createSlice, PayloadAction } from "@reduxjs/toolkit";
import { ParSpecialRatingQuotaGroups, RequestState, SpecialQuotaTeam, SpecialRatingAllocation } from "@utils/types";
...
export const postQuotaGroups = createAsyncThunk(
  ...
  async ({ parCycleId, parSpecialRatingGroups, specialRatingQuotas }: ParSpecialRatingQuotaGroups, { dispatch }) => {
    try {
      ...
      if (response.status === HttpStatusCode.Ok || response.status === HttpStatusCode.Created) {
        dispatch(
          enqueueSnackbarMessage({
            message: snackMessages.success.quotaSaved,
            type: "success",
          })
        );
      ...
      const errorMessage = getErrorMessage(error, snackMessages.error.common);
      dispatch(
        enqueueSnackbarMessage({
...
export const fetchQuotaGroups = createAsyncThunk(
  ...
  async ({ parCycleId, signal }: GetGroupRequest, { dispatch }) => {
    try {
      ...
      const errorMessage = getErrorMessage(error, snackMessages.error.fetchQuotaData);
