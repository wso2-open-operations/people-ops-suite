...
declare global {
  interface Window {
    nativebridge?: {
      ...
      _tokenQueue?: Array<Callback<string>>;
      _tokenResolverInstalled?: boolean;
    };
    ReactNativeWebView?: {
...
export const getToken = (callback: Callback<string>): void => {
  if (window.nativebridge) {
    window.nativebridge._tokenQueue ??= [];
    const queue = window.nativebridge._tokenQueue;
    const shouldRequestToken = queue.length === 0;

    queue.push(callback);

    if (!window.nativebridge._tokenResolverInstalled) {
      window.nativebridge.resolveToken = (token: string) => {
        const pending = window.nativebridge?._tokenQueue ?? [];
        window.nativebridge!._tokenQueue = [];
        pending.forEach((queuedCallback) => queuedCallback(token));
      };
      window.nativebridge._tokenResolverInstalled = true;
    }
 ...
 */
const triggerSuperAppAction = (topic: Topic, data?: unknown): void => {
 ...
 */
export const sendNativeLog = (message?: unknown, data?: unknown, level: LogLevel = "debug"): void => {
  if (window.ReactNativeWebView) {
