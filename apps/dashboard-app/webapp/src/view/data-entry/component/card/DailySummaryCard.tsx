...
function DailySummaryCard(props: DailySummaryCardProps) {
  ...
  const breakfastWaste = toNumber(props.breakfastData.totalWasteKg, 0);
  const lunchWaste = toNumber(props.lunchData.totalWasteKg, 0);
  const breakfastPlates = toNumber(props.breakfastData.plateCount, 0);
  const lunchPlates = toNumber(props.lunchData.plateCount, 0);

  const totalWaste = breakfastWaste + lunchWaste;
  const totalPlates = breakfastPlates + lunchPlates;
        ...
        >
          <Box>
            ...
            <Typography variant="h4">
              {totalWaste.toFixed(1)} {DataEntryMessage.units.kilograms}
            </Typography>
          </Box>
          <Box>
            <Typography variant="body2" color="text.secondary" gutterBottom>
