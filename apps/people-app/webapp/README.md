...
import { getAllowedRoutesV2, routes } from "../route";
```
...
const router = createBrowserRouter([
  {
    ...
    element: <Layout />,
    errorElement: <Error />,
    children: getAllowedRoutesV2(routes, auth.roles),
  },
]);
```
...
- We create a **router tree**:
  ...
  - `children` are the **actual pages**, filtered by `getAllowedRoutesV2(...)` using the user’s **roles**. This is where **authorization** happens for navigation: only allowed routes are included.
