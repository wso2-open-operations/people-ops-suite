import { useEmployeeInfoCreationForm } from "@root/src/hooks/useEmployeeInfoCreationForm";
...
interface EmployeeInfoCreationFormProps {
  editing: boolean;
  toggleEditing: () => void;
}

function EmployeeInfoCreationForm(props: EmployeeInfoCreationFormProps) {
  const { toggleEditing, editing } = props;
  const { form, companiesOptions, buOptions, teamOptions, subTeamOptions, officeOptions, auth } =
    useEmployeeInfoCreationForm();

  const hasAdmin = auth.roles.includes("ADMIN");

  const FormSeperator = () =>
    !editing ? <div className="w-full h-[0.5px] bg-st-border-light"></div> : null;
  ...
  return (
    <form
      ...
      className={`flex flex-col ${!editing ? "gap-3" : "gap-5"}`}
    >
      <form.Field
        name="businessUnitId"
        listeners={{
...
export default EmployeeInfoCreationForm;
